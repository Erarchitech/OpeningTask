    ...
    public class IntersectionService
    {
            ...
            HostElementType hostType)
        {
            try
            {
                ...
                {
                    // Для перекрытия: если MEP почти в плоскости перекрытия (перпендикулярен нормали),
                    // то это не пробивка, а «заходит/идет вдоль» — пропускаем.
            ...
            Solid intersectionSolid)
        {
            ...
            // По умолчанию вычисляем точку вставки как пересечение оси MEP с плоскостью Host.
            // Это устойчивее, чем центроид boolean-пересечения, когда в хосте есть отверстия/вырезы.
            ...
            XYZ nearPoint)
        {
            try
            {
                ...
                // Плоскость Host: нормаль = hostNormal, точка на плоскости = nearPoint.
